<script>
	function check_input(){
		...
		if (!document.member.pass.value != document.member.pass_confirmm.value) {
	...
	function reset_form(){
		...
		document.member.pass_confirmm.value="";
	...
	function check_id(){
		window.open("check_id.php?id=" + document.member.id.value, "IDcheck","left = 700,top=300,width=300,height=160,scrollbarrs=no,resizable=yes");
	}
</script>
<form name="member" action="insert.php" method="post">
	<div class=join_form>
		...
		<ul class="button">
			<li><button type="button" onclick="chech_input()">저장하기</button></li>
